...
from main.serializers import Customer2Serializer


# Create your views here.
def home(request):
  users = Customer.objects.all()
  serializer = Customer2Serializer(users, many=True)
...
def create_customer(request):
  if request.method == 'POST':
    ...
    serializer = Customer2Serializer(data=python_data)
...
def customer_api(request):
  if request.method == "GET":
    ...
    if id is not None:
      ...
      serializer = Customer2Serializer(customer)
    else:
      customer = Customer.objects.all()
      serializer = Customer2Serializer(customer, many=True)
  ...
  if request.method == 'PUT':
    ...
    serializer = Customer2Serializer(customer, data=python_data)

    #* Partial Update - All Data is not required
    # serializer = Customer2Serializer(customer, data=python_data, partial=True)
    if serializer.is_valid():
      serializer.save()

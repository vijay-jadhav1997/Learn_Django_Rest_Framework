...
class StudentAPI(APIView):
  """
    List all students, create a new student, update or delete instance of a student.
  """
  def get(self, request, stud_id=None, format=None):
    if stud_id is not None:
      student = Student.objects.get(pk=stud_id)
      serializer = StudentSerializer(student)
    else:
  ...
  def post(self, request, stud_id=None, format=None):
    serializer = StudentSerializer(data=request.data)
    if serializer.is_valid():
      serializer.save()
      return Response({'message':'New student admission is completed successfully ... 🌼🤾🏻‍♂️'}, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    
  def put(self, request, stud_id=None, format=None):
    student = Student.objects.get(pk=stud_id)
    serializer = StudentSerializer(student, data=request.data)
    if serializer.is_valid():
      serializer.save()
  ...
  def patch(self, request, stud_id=None, format=None):
    student = Student.objects.get(pk=stud_id)
    serializer = StudentSerializer(student, data=request.data, partial=True)
    if serializer.is_valid():
      serializer.save()
  ...
  def delete(self, request, stud_id, format=None):
    student = Student.objects.get(pk=stud_id)
    student.delete()
    return Response({'message': f"The student id '{stud_id}' is deleted successfully... ❌💯"})
